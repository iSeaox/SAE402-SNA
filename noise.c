...
/*********************** self documentation **********************/
/*****************************************************************************
noise - functions to generate a pseudo-random float normally distributed
		with N(0,1); i.e., with zero mean and unit variance.

noise		return a normally distributed random float
snoise		seed random number generator for normal distribution

******************************************************************************
Function Prototypes:
float noise (void);
void snoise (int seed);
...
float noise(void)
{
	...
	if(s<=C2) 
	{
		...
		{
			if(exp(-0.5*v[k+1]*v[k+1])+y*PC/v[k+1] <= exp(-0.5*rnor*rnor)) return rnor;
			do 
			{
				UNI(y);
				x = OXN*log(y);
